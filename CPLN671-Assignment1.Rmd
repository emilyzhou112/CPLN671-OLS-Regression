...
The stepwise regression model confirms the importance of all variables. Removing any variable increases the AIC, indicating a worse model fit. The lowest AIC occurs when all four variables are included, so the final model retains all the predictors from the initial model, confirming their relevance.
...
The original model, which includes all four predictors, has a lower RMSE (0.3676) compared to the reduced model with only two predictors (0.4424). This indicates that the original model provides better predictive accuracy. The increase in RMSE when removing the additional predictors (PCTSINGLES and LNNBELPOV100) suggests that excluding these variables results in a less accurate model. Overall, the original model outperforms the reduced model, demonstrating that all four predictors contribute to improved model accuracy.
